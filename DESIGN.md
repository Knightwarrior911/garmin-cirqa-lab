...
Train is running-only: **Today / This week / Race plan**. Without an imported plan, first use provides one setup call to action. With a private prescription, **My plan** replaces setup and makes every saved week reachable. Today prioritizes the selected session, known duration or explicit mixed steps, specified work distance, full main-work pace/speed range and source HR range. Never present work-only distance as a complete-session estimate. The unit toggle changes presentation only. Recovery and this week's recorded distance sit alongside the decision on desktop and below it on phones. Ladder owns strength; no strength launchers, equipment or routine builder.
...
The optional phone workout guide snapshots an eligible recommendation, showing each step's full pace/speed/HR ranges when supplied. Fixed time uses a large foreground countdown; distance and unspecified-duration steps stay manual. A source duration range remains a range until the owner chooses an optional timer within it. Controlled repeats expose individual work/recovery steps. Timers stop at zero; the user advances manually. Closing pauses; leaving while running warns. Finish creates neither an activity nor plan completion. No background audio/notifications, live HR/pace, distance measurement or CIRQA recording control. A new blocked recommendation removes the guide. Symptoms remain a reason to stop, never something a timer can clear.
...
Imported plans use compact week accordions and session cards with full targets, expandable steps/source notes, bounded date assignment, **Use today**, explicit completion and reopening. Preserve date drafts on failure/refresh. The current source-defined week and native Monday–Sunday mileage have separate headings. Confirmation may link an existing Garmin recording, but progress is never kilometres. Recovery adjustments leave an expandable **Original planned session · unchanged**. Use range-friendly wrapping and two-column metrics rather than truncating numeric targets on narrow phones.
