...
- Save your goal, sport, training weekdays, time, experience and available equipment. Strength requires your existing routine and loads; CIRQA does not invent them. Then complete today's fatigue/soreness and pain/illness check-in. No profile or symptom answers are assumed.
- The planner explains its decision and versioned rules. Pain/illness, substantial symptoms, rest days and an already-recorded session can prevent a prescription. Readiness must be measured today within 12 hours; missing or old data is not interpreted as recovery. Cardio stays conversational and bounded by available time/recent same-sport duration. There is no automatic progression, hard-interval prescription or validated race coaching.
...
**Refresh saved data** reads the stored snapshot; it does not request a Garmin sync. Use **Sync now** on the overview after syncing your watch to Garmin Connect. Training forms preserve unsaved edits across reads. Cloud saves persist in the private snapshot with conditional ETag writes; an active sync or concurrent update may ask you to retry rather than overwrite newer data.
