...
GET `/api/training?period=7|28` returns native load/comparisons, feedback, today's `recovery`, `running` week/baseline/race/dated race-plan/recent-runs context, and the current `recommendation`. Recommendations include total minutes, estimated `distance_km`, `pace_s_per_km`, `speed_kmh`, step targets and disclosed adjustments. POST JSON to `/api/training/profile`, `/api/training/feedback` or `/api/training/checkin` saves owner inputs. All three use the same authentication/origin/request-header protections; validation failures leave earlier inputs intact. They never alter native Garmin scores.
